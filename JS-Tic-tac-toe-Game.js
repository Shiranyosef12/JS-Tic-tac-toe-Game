...
function runGame() {
  ...
  function move(field, x, y, who) {
    ...
    e.value = who
    e.element.innerHTML = who == you ? '<b>X</b>' : '<b>O</b>'
    return true
}
  ...
  function aiMove(field) {
    var m = findBestMove(field, computer).move
    if (!m) {
      gameOver("No one won!")
      return
    }
    move(field, m.x, m.y, computer)
    wins(field, computer) && gameOver("Computer won!")
  }
  runGame()